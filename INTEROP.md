...
How **sbomgate** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `sbomgate` lives in the **Cloud / supply-chain hardening** cluster.
...
| from | relation | to |
|---|---|---|
| `sbomgate` | JSON findings compose with | [`sbomgen`](https://github.com/cognis-digital/sbomgen), [`cvecheck`](https://github.com/cognis-digital/cvecheck), [`k8sharden`](https://github.com/cognis-digital/k8sharden), [`tfscan`](https://github.com/cognis-digital/tfscan) |
| `sbomgate` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `sbomgate` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `sbomgate` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the cloud / supply-chain hardening cluster.** `sbomgate` output feeds the next tool:
```bash
sbomgate ... --format json > out.json          # this tool's findings (see `sbomgate --help`)
...
sbomgate ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
sbomgate ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
sbomgate ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `sbomgate`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `sbomgate` slots
