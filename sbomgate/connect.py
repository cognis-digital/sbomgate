"""Native cognis-connect emit for sbomgate — forward findings to any platform.

Maps sbomgate's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    sbomgate ... --format json | sbomgate-emit --to stix
    sbomgate-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('component', '') + ' ' + rec.get('name', '')
        out['severity'] = rec.get('severity', 'unknown')
