...
**sbomgate** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `sbomgate` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
sbomgate ... --format json | cognis-connect emit --to sigma   # Sigma rules
sbomgate ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
sbomgate ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
sbomgate ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `sbomgate` outputs:

```python
from cognis_connect import normalize, sigma
findings = [normalize(rec, source="sbomgate") for rec in records]   # records = your JSON output
